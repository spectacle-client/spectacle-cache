...
export async function StageInstanceCreate(broker: GatewayBroker, data: string) {
...
export async function StageInstanceUpdate(broker: GatewayBroker, data: string) {
    const parsed = JSON.parse(data) as GatewayStageInstanceCreateDispatchData;
    const key = `${entity}:${parsed.guild_id}:${parsed.id}`;
    await update(broker, entity, key, parsed);
}

export async function StageInstanceDelete(broker: GatewayBroker, data: string) {
    const parsed = JSON.parse(data) as GatewayStageInstanceCreateDispatchData;
    const key = `${entity}:${parsed.guild_id}:${parsed.id}`;
